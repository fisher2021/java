...
<body>
    <div class="page-container">
    	<form action="${ctx }/back/data/userPage?fileId=${fileId }" method="post">
		        		...
		        		<c:forEach items="${page.list }" var="user" varStatus="status">
		        			<tr class="text-c">
					            ...
					            <td  class="<c:if test="${!form.userHave }"> operate</c:if> td-manage">
                                    <a title="删除" href="javascript:;" onclick="admindel('${ctx}/back/data/ajax/deleteUser?id=${user.id }&fileId=${fileId }',this)" class="ml-5" ><i class="Hui-iconfont i-font-size-mid">&#xe6e2;</i></a>
                    			</td>
					        </tr>
		        		</c:forEach>
		        	</c:when>
        ...
        <div class="btn_column radius <c:if test="${form.userHave }"> operate</c:if>">
            <button type="button" class="btn btn-danger radius" onclick="addMore('${ctx}','${ctx }/back/data/ajax/addBatch?fileId=${fileId }')">添加指派对象</button>
        </div>
        <div class="clearfix">

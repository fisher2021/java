...
<body>
    <nav class="breadcrumb">
        <i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 待办事项管理
    ...
    <div class="page-container">
    	<form action="${ctx }/back/backlog/userPage?backlogId=${backlogId }" method="post">
		        	...
		        	<c:when test="${not empty page.list }">
		        		<c:forEach items="${page.list }" var="user" varStatus="status">
		        			<tr class="text-c">
                                ...
                                <%--<td><img src="${ctx }${user.image }" alt="" width="100px" height="100px"></td>--%>
					            <td>${user.nickname }</td>
                                <td>${user.account }</td>
					            <td  class="<c:if test="${!form.userHave }"> operate</c:if> td-manage">
                                    <a title="删除" href="javascript:;" onclick="admindel('${ctx}/back/backlog/ajax/deleteUser?id=${user.id }&backlogId=${backlogId }',this)" class="ml-5" ><i class="Hui-iconfont">&#xe6e2;</i></a>
                    			</td>
					        </tr>
		        		</c:forEach>
		        	</c:when>
        ...
        <div class="btn_column radius <c:if test="${form.userHave }"> operate</c:if>">
            <button type="button" class="btn btn-danger radius" onclick="addMore('${ctx}','${ctx }/back/backlog/ajax/addBatch?backlogId=${backlogId }')">添加指派对象</button>
        </div>
        <div class="clearfix">
